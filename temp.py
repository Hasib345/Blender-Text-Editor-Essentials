...
class AddonTemplateGeneratorOp(bpy.types.Operator):
    ...
    p_name: StringProperty(name = "Name", description="Used for various places", default="Hello World")
    ...
    p_panelOptions: EnumProperty(items = [('DEFAULT_CLOSED', 'Default Closed', 'Defines if the panel has to be open or collapsed at the time of its creation'),
                                           ('HIDE_HEADER', 'Hide Header', 'If set to False, the panel shows a header, which contains a clickable arrow to collapse the panel and the label')],
                                  name="PanelOptions",
    ...
    def invoke(self, context, event):
        return context.window_manager.invoke_props_dialog(self, width=250) # returns {'RUNNING_MODAL'} and calls execute() when 'OK' button is pressed
...
class AddSnippetPanel(GeneratorButtonsPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(pps, 'isClipboard', text="Copy to Clipboard")
...
class TEXT_PT_generator_properties(GeneratorButtonsPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box = col.box()
        box.prop(pps, 'propOptions', text="Options")#, expand=True)
        box.prop(pps, 'propSubtype', text="Subtype")
        box.prop(pps, 'propVecSubtype', text="VecSubType")

        #--------
        split = layout.split()
        row = layout.row(align = True)
        row.operator(AddSnippetOp_Props.bl_idname, text = "Bool", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'Bool'
        row.operator(AddSnippetOp_Props.bl_idname, text = "BoolVec", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'BoolVector'
        layout.separator()
        row = layout.row(align = True)
        row.operator(AddSnippetOp_Props.bl_idname, text = "Int", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'Int'
        row.operator(AddSnippetOp_Props.bl_idname, text = "IntVec", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'IntVector'
        layout.separator()
        row = layout.row(align = True)
        row.operator(AddSnippetOp_Props.bl_idname, text = "Float", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'Float'
        row.operator(AddSnippetOp_Props.bl_idname, text = "FloatVec", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'FloatVector'
        row = layout.row(align = True)
        ...
        layout.separator()
        row = layout.row(align = True)       
        row.operator(AddSnippetOp_Props.bl_idname, text = "Collection", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'Collection'
        row.operator(AddSnippetOp_Props.bl_idname, text = "Pointer", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = 'Pointer'
        layout.separator()
...
class TEXT_PT_generator_panel_place(GeneratorButtonsPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        subSplit = split.split(factor=0.9)
        ...
        row.operator(AddSnippetOp_Samples.bl_idname, text = "", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = "PanelPlace"
        layout.separator()
...
class TEXT_PT_generator_code_samples(GeneratorButtonsPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.prop(pps, 'addonParts', text="Addon Parts")
        if not pps.addonParts.startswith('SEPA'):
            row.operator(AddSnippetOp_Samples.bl_idname, text = "", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = "AddonParts"
        else:
        ...
        col = layout.column(align = True)
        row = col.row(align = True)
        row.prop(pps, 'hintSnippets', text = "Hint Snippets")
        if not pps.hintSnippets.startswith('SEPA'):
            row.operator(AddSnippetOp_Samples.bl_idname, text = "", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = "HintSnippets"
        else:
            row.operator(AddSnippetOp_Samples.bl_idname, text="", icon ="LIBRARY_DATA_INDIRECT").type = "INEFFECTIVE"
        ...
        col = layout.column()
        row = col.row()
        row.prop(pps, 'uiLayoutMembers', text="UILayout Members")
        if not pps.uiLayoutMembers.startswith('SEPA'):
            row.operator(AddSnippetOp_Samples.bl_idname, text = "", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = "UILayoutMembers"
        else:
            row.operator(AddSnippetOp_Samples.bl_idname, text="", icon ="LIBRARY_DATA_INDIRECT").type = "INEFFECTIVE"
        split = layout.split(factor=0.9)
        colLeft = split.column()
        colLeft.prop(pps, "isAddUILayoutParams", text = "Include All Parameters")
...
#        if not pps.isToolbarKeymapClosed:
        ...
        colLeft.alignment = 'RIGHT'
        colLeft.prop(pps, "kmName", text = "Region")
        colLeft.label(text="(%s, %s, Modal: %s)" % (pps.kmSpaceType, pps.kmRegionType, pps.kmIsModal))
#        if not pps.kmIsModal:
#            layout.prop(pps, "kmiIdName", text = "Id name")
#        else:
#            layout.prop(pps, "kmiPropVal", text = "Prop Val")
        colLeft.prop(pps, "kmiMapType", text = "Map Type")
        if pps.kmiMapType != 'TEXTINPUT':
            if pps.kmiMapType == 'TIMER':
                colLeft.prop(pps, "kmiType", text = "Type")
            else:
                row = colLeft.row(align = True)
                row.prop(pps, "kmiType", text = "Type")
                row = colLeft.row(align = True)
                row.prop(pps, "kmiValue", text = "Value")
                row = colLeft.row(align = True)
                row.prop(pps, "kmiKeyMod", text = "Modifier")
        ...
        row = colButton.row()
        row.operator(AddSnippetOp_Samples.bl_idname, text = "", icon="COPYDOWN" if pps.isClipboard else "PASTEDOWN").type = "Keymap"

        layout.operator(AddSnippetOp_Samples.bl_idname, text="Check Keymap Conflicts in Console").type = 'CheckKeymapConflicts'
...
class AddSnippetProps(bpy.types.PropertyGroup):
    ...
    overallName: StringProperty(name = "Name", description = "Used for class name of sample codes", default = "Hello World")
    ...
    propOptions: EnumProperty(items = [('HIDDEN', 'Hidden', 'Hidden'),
                                         ('SKIP_SAVE', 'Skip Save', 'Skip Save'),
                                         ('ANIMATABLE', 'Animatable', 'Animatable'),
                                         ('LIBRARY_EDITABLE', 'Library Editable', 'Library Editable')],
                              name = "PropOptions",
                              description = "Options of Property",
                              default={'ANIMATABLE'},
                              options = {'ENUM_FLAG'})
    ...
    panelSpace: EnumProperty(items = [('VIEW_3D',          '3D View',               '', 'VIEW3D',           0),
                                       ...
                                       ('NODE_EDITOR',      'Compositor',            '', 'NODE_COMPOSITING',8),
                                       ('SEPARATOR2',       '-' * 40,                '', '',                9),
                                       ('PROPERTIES',       'Properties',            '', 'PROPERTIES',      10),
                                       ('USER_PREFERENCES', 'User Preferences',      '', 'PREFERENCES',     11),
                                       ('FILE_BROWSER',     'File Browser',          '', 'FILEBROWSER',     12)],
                              name = "Space",
